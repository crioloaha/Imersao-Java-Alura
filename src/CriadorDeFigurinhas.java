...
public class CriadorDeFigurinhas {
    ...
    public void Criar(InputStream inputStream, String nomeArquivo, String texto, InputStream selo, String classificacao) throws Exception{
        ...
        int seloImageY = altura - 50; // Alterando a posição da imagem selo

        // copiar imagem original para a nova
        Graphics2D graphics = (Graphics2D)novaImagen.getGraphics();
        graphics.drawImage(imgOriginal, 0, 0, null);

        // Redimensionando a imagem selo
        int newWidth = seloImage.getWidth() / 2;
        int newHeight = seloImage.getHeight() / 2;
        Image scaledSelo = seloImage.getScaledInstance(newWidth, newHeight, Image.SCALE_DEFAULT);
        BufferedImage scaledSeloImage = new BufferedImage(newWidth, newHeight, BufferedImage.TRANSLUCENT);
        Graphics2D g2d = scaledSeloImage.createGraphics();
        g2d.drawImage(scaledSelo, 0, 0, null);
        g2d.dispose();

        graphics.drawImage(scaledSeloImage, 0, seloImageY, null);
